...
import {tools,categories,groupByCategory} from '../lib/tools';
...
export default function Home(){
 ...
 return <>
  ...
  <main>
   ...
   <section className="section" id="tools"><div className="wrap"><h2>{results.length?'Recommended for you':'Start with a free tool'}</h2><p className="sectionlead">{results.length?'Based on the problem you described.':'Useful tools are the platform’s traffic and discovery layer.'}</p><div className="groupgrid">{(results.length?groupedResults:groupedFeatured).map(([category,items])=><div className="groupcard" key={category}><span className="tag">{category}</span><h3>{category} Calculators</h3><div className="grouplist">{items.map(t=><Link className="grouplink" href={`/tools/${t.slug}`} key={t.slug}>{t.name}<span>→</span></Link>)}</div></div>)}</div></div></section>
   <section className="section" id="results"><div className="wrap"><h2>The SolveEase flywheel</h2><p className="sectionlead">The platform is designed to turn genuine utility into discovery, recommendations and repeat usage.</p><div className="steps">{[['01','Problem','Tell us what you are trying to solve.','/search'],['02','Diagnose','Use a focused free tool to understand the numbers.','/audits'],['03','Solution','Get the next relevant tool, guide or solution.','/tools'],['04','Return','Save, share and revisit useful business workflows.','/workflows']].map(x=><Link className="card step" href={x[3]} key={x[0]}><span className="tag">{x[0]}</span><strong>{x[1]}</strong><span>{x[2]}</span><small>Explore →</small></Link>)}</div></div></section>
